...
class MainApp(QtGui.QMainWindow):
    ...
    def save_settings(self):
        ...
        if ( self.ui.StreamingConfigBox.isChecked()):
            ...
            port = str(self.ui.portEdit.text())
            password = str(self.ui.passwordEdit.text())
            mount = str(self.ui.mountEdit.text())
            
            if ( url == "" or port == "" or password == "" or mount == ""):
                QtGui.QMessageBox.warning(self, self.tr( "Incomplete Streaming Settings", "Please ensure that all the input fields for streaming are complete or disable the streaming option") , QtGui.QMessageBox.Ok);
            else:
